#' @title Elliptical Slice Sampling for Dirichlet-Laplace Prior

#' @description Elliptical slice sampling for a generic likelihood under a
#'  Dirichlet-Laplace prior for the parameters. 

#' @usage ess_DirLapl = function(nsim, logL, alpha, start, burn = 0, thin = 1,
#'                               verbose = +Inf)
...
#' @import statmod
#' @export
ess_DirLapl = function(
  nsim, logL, alpha, start = NULL, burn = 0, thin = 1, verbose = +Inf
) {

  ### print start time if required
  if (!is.infinite(verbose)) {
    print(paste(
      "ess_DirLapl: start time at ", Sys.time(), sep = ""
    ))
  }
  ...
  if (!is.infinite(verbose)) {
    print(paste(
      "ess_DirLapl: end time at ", Sys.time(), sep = ""
    ))
  }
