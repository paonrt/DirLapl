#' @title Linear Model with Mixture of Dirichlet-Laplace Priors: Markov Chain
#'  Monte Carlo Algorithm

#' @description Gibbs sampler algorithm for the linear model with no intercept
#'  and unknown variance. A mixture of Dirichlet-Laplace prior is used for the
#'  vector of the coefficients and an inverse gamma prior for the variance. 

#' @usage lm_mixDirLapl_mcmc = function(nsim, X, y, alpha, shape_prior,
#'                                      rate_prior, start = NULL, burn = 0,
#'                                      thin = 1, verbose = +Inf)
...
#' coefficient vector \eqn{\beta} is a mixture of the Dirichlet-Laplace priors.
#' Conditionally on the hyperparameter \eqn{\alpha}, the prior is
#' Dirichlet-Laplace that is:
#' \deqn{
...
#' \code{seq(from = 1 / p, to = 0.5, length.out = 200)}. The prior of
#' \eqn{\sigma^2} is an inverse gamma distribution with shape
#' \code{shape_prior} and rate \code{rate_prior}.
#' For the Gibbs sampler, it is used the first representation of the
...
#' @import statmod
#' @export
lm_mixDirLapl_mcmc = function(
  nsim, X, y, shape_prior, rate_prior,
  ...
  if (!is.infinite(verbose)) {
    print(paste(
      "lm_mixDirLapl_mcmc: start time at ", Sys.time(), sep = ""
    ))
  }
  ...
  for (insim in (1 - burn):nsim) {
    ...
    while (ithin < thin) {
      ...
      log_wei_alpha = log_const_alpha + log_kernel_alpha(
        abs_theta = drop(abs(beta) / sqrt(sigmaSq)),
        seq_alpha = seq_alpha, p = p
      )
  ...
  if (!is.infinite(verbose)) {
    print(paste(
      "lm_mixDirLapl_mcmc: end time at ", Sys.time(), sep = ""
    ))
  }
